    ...
    if (NumA.length) {
      ...
      if (NumBpick[0].title) {
        ...
        if (matchesActual.length) {
          console.log(NumBpick[0].nums);

          var numberHitsCheck = numberHits.find({title: tempTitle[0].game, date: tempTitle[0].drawdate,
            datepicked: NumBpick[0].date, picked: NumBpick[0].nums, actual: tempTitle[0].winnum,
            matches: matchesActual}).fetch();

          console.log(numberHitsCheck);

          if (numberHitsCheck < 1) {
            console.log("in the if");
            Meteor.call("numberHits", tempTitle[0].game, tempTitle[0].drawdate, NumBpick[0].date, NumBpick[0].nums, tempTitle[0].winnum, matchesActual, function (error, result) {
              if (error) {
